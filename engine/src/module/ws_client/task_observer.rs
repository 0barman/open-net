...
impl TaskObserverStore {
    ...
    {
        on_common::log_t!(LogType::WSC; "register_task_listener", "instance_id", self.instance_id);
    ...
    pub(crate) fn unregister(&self) -> Result<(), NetError> {
        on_common::log_t!(LogType::WSC; "unregister_task_listener", "instance_id", self.instance_id);
    ...
    pub(crate) fn close(&self) {
        on_common::log_t!(LogType::WSC; "close_task_listener", "instance_id", self.instance_id);
        let previous = {
            let mut state = match self.state.lock() {
                Ok(state) => state,
                Err(poisoned) => {
                    on_common::log_e!(LogType::WSC; "close_task_listener", "error", "store_lock_poisoned_recovered");
                    poisoned.into_inner()
                }
            };
            state.closed = true;
    ...
    fn lock_state(&self) -> Result<MutexGuard<'_, ObserverStoreState>, NetError> {
        self.state.lock().map_err(|_| {
            on_common::log_e!(LogType::WSC; "lock_task_observer_store", "error", "store_lock_poisoned");
...
impl TaskListenerRegistration {
        ...
        S: FnOnce(Box<dyn FnOnce() + Send>) -> io::Result<()>,
    {
        ...
        let task = Box::new(move || {
            ...
            while let Ok(dispatched) = receiver.recv() {
                ...
                if catch_unwind(AssertUnwindSafe(|| listener(event))).is_err() {
                    on_common::log_e!(LogType::WSC; "dispatch_task_event", "error", "user_callback_panicked");
                }
        ...
        spawn(task).map_err(|error| {
            on_common::log_e!(LogType::WSC; "register_task_listener", "error", on_common::log::summary::error(&error));
            NetError::IOError
        })?;
        Ok(Self {
            sender,
...
impl TaskObservation {
    ...
    pub(crate) fn finish(&self, result: Result<Success, NetError>) {
        let state = {
            ...
            if state.is_some() && self.selected_result.set(result).is_err() {
                on_common::log_e!(LogType::WSC; "finish_task_observation", "error", "terminal_result_already_selected");
            }
            state
        };
        ...
        let state = {
                ...
                .is_some_and(|state| state.event.phase == Phase::WaitingForCapacity)
            {
                ...
                if self.selected_result.set(Err(error)).is_err() {
                    on_common::log_e!(LogType::WSC; "finish_waiting_task_observation", "error", "terminal_result_already_selected");
                }
                selected
            } else {
                None
    ...
    fn lock_state(&self) -> MutexGuard<'_, Option<ObservationState>> {
        match self.state.lock() {
            Ok(state) => state,
            Err(poisoned) => {
                on_common::log_e!(LogType::WSC; "lock_task_observation", "error", "observation_lock_poisoned_recovered");
                poisoned.into_inner()
            }
        }
    }
...
impl Drop for TaskObservation {
    fn drop(&mut self) {
        let state = match self.state.get_mut() {
            Ok(state) => state.take(),
            Err(poisoned) => {
                on_common::log_e!(LogType::WSC; "drop_task_observation", "error", "observation_lock_poisoned_recovered");
        ...
        if let Some(state) = state {
            if self
                ...
                .is_err()
            {
                on_common::log_e!(LogType::WSC; "drop_task_observation", "error", "terminal_result_already_selected");
            }
...
impl ObservationState {
    fn finish(mut self, result: Result<Success, NetError>) {
        ...
        if sender.send(event).is_err() {
            ...
            on_common::log_e!(LogType::WSC; "finish_task_observation", "task_id|error", task_id, "task_dispatcher_unavailable");
        }
    }
}
    ...
    fn blocked_callback_keeps_capacity_and_queued_notifications_survive_close() -> TestResult {
        ...
        store.register(Box::new(move |event| {
            ...
            match release_rx.lock() {
                ...
                Err(_) => { on_common::log_e!(LogType::WSC; "blocked_callback_test", "error", "release_lock_poisoned"); }
            }

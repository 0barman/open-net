...
async fn send_read_end(io_event_tx: &mpsc::Sender<IoEvent>, generation: u64, error: NetError) {
    on_common::log_t!(LogType::WSC; "send_read_end", "generation|error", generation, format!("{error:?}"));
    if matches!(error, NetError::ConnectionClosed | NetError::Cancelled) {
        on_common::log_s!(LogType::WSC; "send_read_end", "state|generation|error", "read_ended", generation, format!("{error:?}"));
    } else {
        on_common::log_e!(LogType::WSC; "send_read_end", "generation|error", generation, format!("{error:?}"));
    }
    if io_event_tx
        ...
        .is_err()
    {
        on_common::log_s!(LogType::WSC; "send_read_end", "state|generation", "worker_event_receiver_closed", generation);
    }
}
...
{
    on_common::log_t!(LogType::WSC; "run_read_loop", "generation|max_callback_bytes", context.generation, context.data_callback_max_bytes);
    let ReadLoopContext {
        control_tx,
        data_callback_tx,
        data_callback_bytes,
    ...
    loop {
        let next = tokio::select! {
            _ = cancel.cancelled() => {
                on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation", "read_cancelled", generation);
                return;
            },
            next = read.next() => next,
        };
        let Some(next) = next else {
            on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation", "stream_ended", generation);
        ...
        let message = match next {
            Ok(message) => message,
            Err(error) => {
                on_common::log_e!(LogType::WSC; "run_read_loop", "generation|error", generation, on_common::log::summary::error(&error));
                send_read_end(&io_event_tx, generation, error.into()).await;
                return;
            }
        };

        on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation|message_type|message_bytes", "message_received", generation, match &message { Message::Text(_) => "text", Message::Binary(_) => "binary", Message::Ping(_) => "ping", Message::Pong(_) => "pong", Message::Close(_) => "close", Message::Frame(_) => "frame" }, message.len());
        match message {
            Message::Text(_) | Message::Binary(_) => {
                let payload_bytes = message.len().max(1);
                if payload_bytes > data_callback_max_bytes {
                    on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation|payload_bytes|max_bytes", "callback_payload_too_large", generation, payload_bytes, data_callback_max_bytes);
                    ...
                    .try_acquire_many_owned(payload_bytes as u32)
                {
                    Ok(permit) => permit,
                    Err(_) => {
                        on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation|payload_bytes", "callback_byte_budget_exhausted", generation, payload_bytes);
                ...
                let listener = listeners
                    ...
                    .unwrap_or_else(|_| {
                        on_common::log_e!(LogType::WSC; "run_read_loop", "error", "data_listener_lock_poisoned");
                        None
                    });
                ...
                match data_callback_tx.try_send(CallbackEvent::Data {
                    ...
                    Ok(()) => {
                        on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation|message_bytes", "data_callback_queued", generation, payload_bytes);
                    }
                    Err(mpsc::error::TrySendError::Full(_)) => {
                        on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation", "data_callback_lane_full", generation);
                        send_read_end(&io_event_tx, generation, NetError::CallbackQueueOverflow)
                            .await;
                        return;
                    }
                    Err(mpsc::error::TrySendError::Closed(_)) => {
                        on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation", "data_callback_lane_closed", generation);
            ...
            Message::Ping(_) => match control_tx.try_send(ControlMessage::FlushAutomatic) {
                Ok(()) => {}
                Err(mpsc::error::TrySendError::Full(_)) => {
                    on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation", "automatic_pong_control_lane_full", generation);
            ...
            Message::Pong(payload) => {
                ...
                on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation|matched", "pong_received", generation, matched);
            }
            Message::Close(_) => {
                ...
                match control_tx.try_send(ControlMessage::PeerClose(done_tx)) {
                    ...
                    Err(mpsc::error::TrySendError::Full(_)) => {
                        on_common::log_e!(LogType::WSC; "run_read_loop", "stage|generation", "peer_close_control_lane_full", generation);
                        send_read_end(&io_event_tx, generation, NetError::ControlQueueOverflow)
                            .await;
                        return;
                    }
                    Err(mpsc::error::TrySendError::Closed(_)) => {
                        on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation", "peer_close_control_channel_closed", generation);
                    }
                }
                send_read_end(&io_event_tx, generation, NetError::ConnectionClosed).await;
                return;
            ...
            Message::Frame(_) => {
                on_common::log_s!(LogType::WSC; "run_read_loop", "state|generation", "raw_frame_ignored", generation);
            }
        }
    }
}

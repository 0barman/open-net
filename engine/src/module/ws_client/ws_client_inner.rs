...
impl DispatchCancellationGuard {
    fn new(
        ...
        on_common::log_t!(LogType::WSC; "new");
        Self {
            cancel,
            phase,
            pending,
    ...
    fn mark_enqueued(&mut self, queue: Arc<PriorityWriteQueue>) {
        on_common::log_t!(LogType::WSC; "mark_enqueued");
        self.queue = Some(queue);
    }

    fn disarm(&mut self) {
        on_common::log_t!(LogType::WSC; "disarm");
        self.armed = false;
        self.pending.take();
    }
}
    ...
    fn drop(&mut self) {
        ...
        match self.phase.cancel() {
            DispatchCancellation::Queued => {
                ...
                on_common::log_s!(LogType::WSC; "drop", "stage", "cancel_queued_dispatch");
                self.cancel.cancel();
            ...
            DispatchCancellation::Writing => {
                ...
                on_common::log_s!(LogType::WSC; "drop", "stage", "cancel_in_flight_dispatch");
...
impl WSClientInner {
    ...
    pub(crate) fn new_with_network(
        ...
        on_common::log_t!(LogType::WSC; "new", "config", format!("{:?}", config));
        let result: Result<(Arc<Self>, WSClientWorker), NetError> = (|| {
            ...
            on_common::log_s!(LogType::WSC; "new", "status", "Idle");
            Ok((inner, worker))
        })();
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "new", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub(crate) fn register_data_listener(&self, listener: WebSocketClientDataReceiveListener) {
        on_common::log_t!(LogType::WSC; "register_data_listener", "listener", "callback");
        if let Ok(mut current) = self.listeners.data.write() {
            ...
            on_common::log_e!(LogType::WSC; "register_data_listener", "error", "data_listener_lock_poisoned");
        }
    }
    ...
    pub(crate) fn unregister_data_listener(&self) {
        on_common::log_t!(LogType::WSC; "unregister_data_listener");
        if let Ok(mut current) = self.listeners.data.write() {
            current.take();
        } else {
            on_common::log_e!(LogType::WSC; "unregister_data_listener", "error", "data_listener_lock_poisoned");
        }
    }
    ...
    pub(crate) fn register_status_listener(&self, listener: WebSocketClientConnectStatusListener) {
        on_common::log_t!(LogType::WSC; "register_status_listener", "listener", "callback");
        ...
        let (terminal, previous) = if let Ok(mut current) = self.listeners.status.write() {
            ...
            on_common::log_e!(LogType::WSC; "register_status_listener", "error", "status_listener_lock_poisoned");
    ...
    pub(crate) fn unregister_status_listener(&self) {
        on_common::log_t!(LogType::WSC; "unregister_status_listener");
        let previous = if let Ok(mut current) = self.listeners.status.write() {
            current.take()
        } else {
            on_common::log_e!(LogType::WSC; "unregister_status_listener", "error", "status_listener_lock_poisoned");
            None
        };
        if let Some(previous) = previous {
            previous.retired.cancel();
    ...
    pub(crate) fn connection_status(&self) -> ConnectionStatus {
        ...
        self.state.read().map(|state| *state).unwrap_or_else(|_| {
            on_common::log_e!(LogType::WSC; "connection_status", "error", "state_lock_poisoned");
            ConnectionStatus::Closed
        })
    }
    ...
    pub(crate) fn last_connection_error(&self) -> Option<NetError> {
        ...
        self.last_connection_error
            ...
            .map(|error| *error)
            .unwrap_or_else(|_| {
                on_common::log_e!(LogType::WSC; "last_connection_error", "error", "snapshot_lock_poisoned");
                None
            })
    }
    ...
    pub(crate) fn last_handshake_http_status(&self) -> Option<u16> {
        ...
        self.last_handshake_http_status
            ...
            .map(|status| *status)
            .unwrap_or_else(|_| {
                on_common::log_e!(LogType::WSC; "last_handshake_http_status", "error", "snapshot_lock_poisoned");
                None
            })
    }
    ...
    pub(crate) async fn connect(
        ...
        on_common::log_t!(LogType::WSC; "connect", "url|header_count|has_header_provider|reconnect", on_common::log::summary::url(&url), options.headers.len(), options.header_provider.is_some(), format!("{:?}", options.reconnect));
    ...
    pub(crate) fn notify_network_available(&self) {
        ...
        let notified = self
            ...
            .is_ok();
        on_common::log_s!(LogType::WSC; "notify_network_available", "command_queued", notified);
    }
    ...
    pub(crate) fn request_shutdown(&self) {
        on_common::log_t!(LogType::WSC; "request_shutdown");
        ...
        on_common::log_s!(LogType::WSC; "request_shutdown", "stage", "shutdown_requested");
        let _ = self
            .command_tx
    ...
    pub(crate) async fn send(
        ...
        on_common::log_t!(LogType::WSC; "send", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
    ...
    pub(crate) async fn send_with_completion(
        ...
        on_common::log_t!(LogType::WSC; "send_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
    ...
    async fn send_internal(
        ...
        on_common::log_t!(LogType::WSC; "send_internal", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        ...
        let body = request.body().inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "send_internal", "stage|error", "build_body", format!("{error:?}"));
        })?;
        let size = body.len();
        ...
        on_common::log_s!(LogType::WSC; "send_internal", "stage|uuid|body_bytes", "body_ready", uuid, size);
        ...
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "send_internal", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub(crate) fn try_send(
        ...
        on_common::log_t!(LogType::WSC; "try_send", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<(), NetError> = (|| {
    ...
    pub(crate) fn try_send_with_completion(
        ...
        on_common::log_t!(LogType::WSC; "try_send_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
    ...
    fn try_send_internal(
        ...
        on_common::log_t!(LogType::WSC; "try_send_internal", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<TrySendAdmission, NetError> = (|| {
            let admission_cancel = self.validate_send_admission(&config)?;
            let uuid = request.uuid();
            let body = request.body().inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "try_send_internal", "stage|error", "build_body", format!("{error:?}"));
        })?;
            let size = body.len();
            if uuid.trim().is_empty() {
                return Err(NetError::ParameterEmpty);
            ...
            on_common::log_s!(LogType::WSC; "try_send_internal", "stage|uuid|body_bytes", "body_ready", uuid, size);
            let message = match body {
                WsBody::Text(value) => Message::Text(value.into()),
                WsBody::Binary(value) => Message::Binary(value),
            };
        ...
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "try_send_internal", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub(crate) async fn send_untracked_scoped(
        ...
        on_common::log_t!(LogType::WSC; "send_untracked", "config|body_bytes|urgent", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() }, urgent);
        let result: Result<(), NetError> = async {
        ...
        .await;
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "send_untracked", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub(crate) fn try_send_untracked_scoped(
        ...
        on_common::log_t!(LogType::WSC; "try_send_untracked", "config|body_bytes|urgent", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() }, urgent);
        let result: Result<(), NetError> = (|| {
            config.expect_response = false;
            let admission_cancel = self.validate_send_admission_scoped(&config, scope.as_ref())?;
            let sequence = NEXT_UNTRACKED_REQUEST_ID.fetch_add(1, Ordering::Relaxed);
        ...
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "try_send_untracked", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    fn validate_send_admission_scoped(
        ...
        on_common::log_t!(LogType::WSC; "validate_send_admission", "config", format!("{:?}", config));
        ...
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "validate_send_admission", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    async fn enqueue_and_wait_scoped(
        ...
        on_common::log_t!(LogType::WSC; "enqueue_and_wait", "config|uuid|pending_token|size", format!("{:?}", config), uuid, pending_token, size);
        let result: Result<(), NetError> = async {
        ...
        on_common::log_s!(LogType::WSC; "enqueue_and_wait", "stage|uuid|pending_token|size", "queued", uuid, pending_token, size);
        ...
        guard.disarm();
        on_common::log_s!(LogType::WSC; "enqueue_and_wait", "stage|uuid|success", "write_resolved", uuid, result.is_ok());
        result
    }.await;
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "enqueue_and_wait", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    fn try_enqueue_scoped(
        ...
        on_common::log_t!(LogType::WSC; "try_enqueue", "config|uuid|pending_token|size", format!("{:?}", config), uuid, pending_token, size);
        let result: Result<oneshot::Receiver<Result<(), NetError>>, NetError> = (|| {
            ...
            guard.disarm();
            on_common::log_s!(LogType::WSC; "try_enqueue", "stage|pending_token|size", "queued", pending_token, size);
            Ok(result_rx)
        })();
        result.inspect_err(|error| {
            on_common::log_e!(LogType::WSC; "try_enqueue", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn set_log_listener(&self, listener: Option<LogListener>) {
        ...
        if let Err(error) = self.try_set_log_listener(listener) {
            on_common::log_e!(LogType::WSC; "set_log_listener", "error", on_common::log::summary::error(&error));
        }
    }
    ...
    pub(crate) fn try_set_log_listener(
        ...
        let previous = {
            let mut current = self.listeners.log.lock().unwrap_or_else(|poisoned| {
                on_common::log_e!(LogType::WSC; "try_set_log_listener", "error", "log_listener_lock_poisoned_recovered");
                poisoned.into_inner()
            });
            let replacement = if self.shutdown.is_cancelled() {
                ...
                listener
                    ...
                    .map(|listener| {
                        ...
                        on_common::log::logger::Logger::register_log_listener(
        ...
        drop(previous);
        on_common::log_s!(LogType::WSC; "try_set_log_listener", "registered", enabled);
        Ok(())
    }
}
    ...
    fn drop(&mut self) {
        ...
        let subscription = self
            ...
            .unwrap_or_else(|poisoned| {
                on_common::log_e!(LogType::WSC; "drop", "error", "log_listener_lock_poisoned_recovered");
                poisoned.into_inner()
            })
            .take();
        drop(subscription);
    ...
    fn status_listener_registered_after_shutdown_is_not_stored_and_observes_closed() -> TestResult {
        ...
        inner.register_status_listener(Box::new(move |status| {
            ...
            if callback_tx.send(result).is_err() {
                on_common::log_e!(LogType::WSC; "test_callback", "error", format!("status callback result receiver closed"));
            }
        }));

...
fn lock_terminal_error(error: &Mutex<Option<NetError>>) -> MutexGuard<'_, Option<NetError>> {
    match error.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            on_common::log_e!(LogType::WSC; "dispatch_terminal_error", "error", "lock_poisoned_recovered");
            poisoned.into_inner()
        }
    }
}
...
fn lock_write_retirement(
    ...
    match retirement.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            on_common::log_e!(LogType::WSC; "dispatch_write_retirement", "error", "lock_poisoned_recovered");
            poisoned.into_inner()
        }
    }
}
...
fn cancel_phase(phase: &AtomicU8) -> DispatchCancellation {
    on_common::log_t!(LogType::WSC; "cancel");
    loop {
        ...
        let (outcome, cancelled) = match current {
            ...
            _ => {
                on_common::log_e!(LogType::WSC; "cancel", "error|phase", "invalid_dispatch_phase", current);
                return DispatchCancellation::AlreadyResolved;
            }
        };
        if phase
            ...
            .is_ok()
        {
            on_common::log_s!(LogType::WSC; "cancel", "old_phase|new_phase|outcome", current, cancelled, format!("{:?}", outcome));
            return outcome;
        }
    }
}
...
impl DispatchPhase {
    ...
    pub(crate) fn set_pending_cleanup(
        ...
        on_common::log_t!(LogType::WSC; "set_pending_cleanup", "pending_requests|uuid|token", "PendingRequestView", uuid, token);
        let mut cleanup = self
            .pending_cleanup
            .lock()
            .unwrap_or_else(|poisoned| { on_common::log_e!(LogType::WSC; "set_pending_cleanup", "error", "lock_poisoned_recovered"); poisoned.into_inner() });
        if cleanup.is_some() {
            on_common::log_e!(LogType::WSC; "set_pending_cleanup", "error|uuid|token", "pending_cleanup_already_registered", uuid, token);
    ...
    fn cleanup_pending(&self, error: NetError) -> bool {
        on_common::log_t!(LogType::WSC; "cleanup_pending", "error", format!("{:?}", error));
        let cleanup = self
            .pending_cleanup
            .lock()
            .unwrap_or_else(|poisoned| { on_common::log_e!(LogType::WSC; "cleanup_pending", "error", "lock_poisoned_recovered"); poisoned.into_inner() })
            .take();
    ...
    fn disarm_pending_cleanup(&self) {
        on_common::log_t!(LogType::WSC; "disarm_pending_cleanup");
        let cleanup = self
            .pending_cleanup
            .lock()
            .unwrap_or_else(|poisoned| { on_common::log_e!(LogType::WSC; "disarm_pending_cleanup", "error", "lock_poisoned_recovered"); poisoned.into_inner() })
            .take();
        if let Some(cleanup) = cleanup {
            cleanup
                .pending_requests
    ...
    pub(crate) fn take_write_response_claim(&self) -> bool {
        on_common::log_t!(LogType::WSC; "take_write_response_claim");
        let cleanup = self
            .pending_cleanup
            .lock()
    ...
    pub(crate) fn pending_view_for_cleanup(&self) -> Option<PendingRequestView> {
        let cleanup = match self.pending_cleanup.lock() {
            Ok(cleanup) => cleanup,
            Err(poisoned) => {
                on_common::log_e!(LogType::WSC; "dispatch_pending_view", "error", "lock_poisoned_recovered");
                poisoned.into_inner()
            }
        };
        cleanup
    ...
    pub(crate) fn set_response_deadline(&self, deadline: Option<std::time::Instant>) {
        let mut current = match self.response_deadline.lock() {
            Ok(current) => current,
            Err(poisoned) => {
                on_common::log_e!(LogType::WSC; "set_dispatch_response_deadline", "error", "lock_poisoned_recovered");
                poisoned.into_inner()
            }
        };
        *current = deadline;
    ...
    pub(crate) fn response_deadline(&self) -> Option<std::time::Instant> {
        match self.response_deadline.lock() {
            Ok(current) => *current,
            Err(poisoned) => {
                on_common::log_e!(LogType::WSC; "dispatch_response_deadline", "error", "lock_poisoned_recovered");
                *poisoned.into_inner()
            }
        }
    }
    ...
    pub(crate) fn start_writing(&self) -> bool {
        on_common::log_t!(LogType::WSC; "start_writing");
        let started = self
            .phase
            .compare_exchange(
                DISPATCH_QUEUED,
            ...
            .is_ok();
        on_common::log_s!(LogType::WSC; "start_writing", "transition|won", "Queued->Writing", started);
        started
    }
    ...
    pub(crate) fn commit(&self) -> bool {
        on_common::log_t!(LogType::WSC; "commit");
        if self
            .response_deadline()
            .is_some_and(|deadline| tokio::time::Instant::now().into_std() >= deadline)
        {
        ...
        let committed = self
            ...
            .is_ok();
        on_common::log_s!(LogType::WSC; "commit", "transition|won", "Writing->Committed", committed);
        committed
    }

    /// 一次可重试失败后把 writer 所有权交回队列。
    pub(crate) fn requeue(&self) -> bool {
        on_common::log_t!(LogType::WSC; "requeue");
        let requeued = self
            .phase
            .fetch_update(AtomicOrdering::AcqRel, AtomicOrdering::Acquire, |current| {
                matches!(current, DISPATCH_WRITING | DISPATCH_DATA_STARTED)
        ...
        on_common::log_s!(LogType::WSC; "requeue", "transition|won", "Writing->Queued", requeued);
        requeued
    }

    /// 发布不再由 writer/队列处理的终态。取消已经赢得竞争时保持 Cancelled。
    pub(crate) fn finish(&self) {
        on_common::log_t!(LogType::WSC; "finish");
        let mut current = self.phase.load(AtomicOrdering::Acquire);
        while matches!(
            current,
            ...
            match self.phase.compare_exchange(
                ...
                Ok(_) => {
                    on_common::log_s!(LogType::WSC; "finish", "old_phase|new_phase", current, DISPATCH_FINISHED);
                    return;
                }
                Err(actual) => current = actual,
            }
    ...
    pub(crate) fn is_cancelled(&self) -> bool {
        on_common::log_t!(LogType::WSC; "is_cancelled");
        matches!(
            self.phase.load(AtomicOrdering::Acquire),
            DISPATCH_CANCELLED_QUEUED
                | DISPATCH_CANCELLED_WRITING
    ...
    fn drop_has_unknown_delivery(&self) -> bool {
        on_common::log_t!(LogType::WSC; "drop_has_unknown_delivery");
        matches!(
            self.phase.load(AtomicOrdering::Acquire),
    ...
    fn finish(&mut self, result: Result<(), NetError>) {
        ...
        if let Some(sender) = self.result_tx.take() {
            if sender.send(result).is_err() {
                on_common::log_s!(LogType::WSC; "deferred_write_completion", "state", "write_receiver_dropped");
            }
        }
    }
}
...
impl Drop for DeferredWriteCompletion {
    fn drop(&mut self) {
        if self.result_tx.is_some() {
            on_common::log_e!(LogType::WSC; "drop_deferred_write_completion", "error", format!("{:?}", self.fallback_error));
...
impl QueuedRequest {
    ...
    pub(crate) fn complete(mut self, result: Result<(), NetError>) {
        ...
        match result {
            Ok(()) => {
                on_common::log_s!(LogType::WSC; "complete", "uuid|sequence|state", self.uuid, self.sequence, "write_or_response_confirmed")
            }
            Err(NetError::Cancelled) => {
                on_common::log_s!(LogType::WSC; "complete", "uuid|sequence|state", self.uuid, self.sequence, "cancelled")
            }
            Err(error) => {
                on_common::log_e!(LogType::WSC; "complete", "uuid|sequence|error", self.uuid, self.sequence, format!("{:?}", error))
            }
        }
        if let Some(result_tx) = self.result_tx.take() {
            if result_tx.send(result).is_err() {
                on_common::log_s!(LogType::WSC; "complete", "uuid|state", self.uuid, "write_receiver_dropped");
            }
        }
    }
    ...
    pub(crate) fn remember_delivery_error(&mut self, error: NetError) {
        on_common::log_t!(LogType::WSC; "remember_delivery_error", "error", format!("{:?}", error));
        ...
        on_common::log_s!(LogType::WSC; "remember_delivery_error", "uuid|prior_delivery_unknown", self.uuid, self.prior_delivery_unknown);
    }

    /// 将一般终止原因提升为此前已经发生的更强投递语义。
    pub(crate) fn terminal_error(&self, fallback: NetError) -> NetError {
        on_common::log_t!(LogType::WSC; "terminal_error", "fallback", format!("{:?}", fallback));
    ...
    fn priority(&self) -> WSRequestPriority {
        on_common::log_t!(LogType::WSC; "priority");
...
impl Drop for QueuedRequest {
    ...
    fn drop(&mut self) {
        ...
        if self.result_tx.is_some() {
            if let Some(pending) = self.dispatch_phase.pending_view_for_cleanup() {
                match pending.defer_registration_write(self) {
                    ...
                    Ok(false) => {}
                    Err(error) => {
                        on_common::log_e!(LogType::WSC; "drop", "stage|error", "defer_response_grace", format!("{error:?}"));
                    }
                }
            }
        }
        ...
        result = result.map_err(|error| self.dispatch_phase.observed_error(error));
        if let Some(result_tx) = self.result_tx.take() {
            on_common::log_s!(LogType::WSC; "drop", "uuid|sequence|state|result", self.uuid, self.sequence, "fallback_completion", format!("{:?}", result));
            if result_tx.send(result).is_err() {
                on_common::log_s!(LogType::WSC; "drop", "uuid|state", self.uuid, "write_receiver_dropped");
            }
        }
    }
}
...
impl PartialEq for QueuedRequest {
    ...
    fn eq(&self, other: &Self) -> bool {
        on_common::log_t!(LogType::WSC; "eq", "sequence|other_sequence|priority|other_priority", self.sequence, other.sequence, format!("{:?}", self.config.priority), format!("{:?}", other.config.priority));
...
impl PartialOrd for QueuedRequest {
    ...
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        on_common::log_t!(LogType::WSC; "partial_cmp", "sequence|other_sequence|priority|other_priority", self.sequence, other.sequence, format!("{:?}", self.config.priority), format!("{:?}", other.config.priority));
        Some(self.cmp(other))
    }
}
...
impl Ord for QueuedRequest {
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        on_common::log_t!(LogType::WSC; "cmp", "sequence|other_sequence|priority|other_priority", self.sequence, other.sequence, format!("{:?}", self.config.priority), format!("{:?}", other.config.priority));
        self.priority()
            .cmp(&other.priority())
...
mod tests {
    ...
    use on_common::log::{LogLevel, Logger};

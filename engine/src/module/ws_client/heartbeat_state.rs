...
impl HeartbeatState {
    ...
    pub(crate) fn new(generation: u64) -> Self {
        on_common::log_t!(LogType::WSC; "new", "generation", generation);
        Self {
            generation,
    ...
    pub(crate) fn on_tick(&self, now: Instant, pong_timeout: Duration) -> HeartbeatTick {
        on_common::log_t!(LogType::WSC; "on_tick", "now|pong_timeout", format!("{:?}", now), format!("{:?}", pong_timeout));
        ...
        match &tick {
            HeartbeatTick::SendProbe(payload) => {
                on_common::log_s!(LogType::WSC; "on_tick", "generation|state|payload_bytes", self.generation, "Sending", payload.len())
            }
            HeartbeatTick::Waiting => {
                on_common::log_s!(LogType::WSC; "on_tick", "generation|state", self.generation, "probe_outstanding")
            }
            HeartbeatTick::TimedOut => {
                on_common::log_e!(LogType::WSC; "on_tick", "generation|error", self.generation, "PongTimeout")
            }
        }
        tick
    }
    ...
    pub(crate) fn mark_sent(&self, payload: &[u8], sent_at: Instant) -> bool {
        on_common::log_t!(LogType::WSC; "mark_sent", "payload_bytes|sent_at", payload.len(), format!("{:?}", sent_at));
        let mut inner = self.lock_inner();
        let Some(outstanding) = inner.outstanding.as_mut() else {
            on_common::log_s!(LogType::WSC; "mark_sent", "generation|state", self.generation, "probe_already_cleared");
            return false;
        };
        if outstanding.payload.as_ref() != payload
            || !matches!(outstanding.phase, ProbePhase::Sending)
        {
            on_common::log_s!(LogType::WSC; "mark_sent", "generation|state", self.generation, "probe_or_phase_mismatch");
            return false;
        }
        outstanding.phase = ProbePhase::AwaitingPong { sent_at };
        drop(inner);
        on_common::log_s!(LogType::WSC; "mark_sent", "generation|state", self.generation, "AwaitingPong");
        true
    }

    /// Clears the current probe only for an exact payload match.
    pub(crate) fn acknowledge_pong(&self, payload: &[u8]) -> bool {
        on_common::log_t!(LogType::WSC; "acknowledge_pong", "payload_bytes", payload.len());
        let now = Instant::now();
        let mut inner = self.lock_inner();
        ...
        drop(inner);
        on_common::log_s!(LogType::WSC; "acknowledge_pong", "generation|accepted", self.generation, matches);
        matches
    }

    /// Removes a probe whose Ping write failed or was cancelled.
    pub(crate) fn abandon_probe(&self, payload: &[u8]) {
        on_common::log_t!(LogType::WSC; "abandon_probe", "payload_bytes", payload.len());
        let mut inner = self.lock_inner();
        if inner
            .outstanding
            .as_ref()
            .is_some_and(|probe| probe.payload.as_ref() == payload)
        {
            inner.outstanding = None;
            drop(inner);
            on_common::log_s!(LogType::WSC; "abandon_probe", "generation|state", self.generation, "probe_abandoned");
        }
    }
    ...
    pub(crate) fn pong_deadline(&self) -> Option<Instant> {
        on_common::log_t!(LogType::WSC; "pong_deadline");
    ...
    pub(crate) fn is_timed_out(&self, now: Instant) -> bool {
        on_common::log_t!(LogType::WSC; "is_timed_out", "now", format!("{:?}", now));
        let timed_out = self.pong_deadline().is_some_and(|deadline| now >= deadline);
        if timed_out {
            on_common::log_e!(LogType::WSC; "is_timed_out", "generation|error", self.generation, "PongTimeout");
        }
        timed_out
    }
...
fn heartbeat_payload(generation: u64, sequence: u64) -> Bytes {
    on_common::log_t!(LogType::WSC; "heartbeat_payload", "generation|sequence", generation, sequence);

...
impl CallbackGate {
    ...
    fn release(&self) {
        let mut released = match self.released.lock() {
            ...
            Err(error) => {
                on_common::log_e!(LogType::WSC; "callback_budget_test_cleanup", "error", error.to_string());
                error.into_inner()
            }
        };
        *released = true;
    ...
    fn listener(
        ...
        Arc::new(move |response| {
            ...
            if let Err(error) = results.try_send(result) {
                on_common::log_e!(LogType::WSC; "callback_budget_test", "error", error.to_string());
            }
        })
    }
    ...
    fn drop(&mut self) {
        ...
        for handle in handles {
            if handle.join().is_err() {
                on_common::log_e!(LogType::WSC; "callback_budget_test_cleanup", "error", "callback_thread_join_failed");
            }
        }
    }
}

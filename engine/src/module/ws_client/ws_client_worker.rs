...
impl WSClientWorker {
    ...
    pub(crate) fn run(self, ready_tx: std::sync::mpsc::Sender<Result<(), NetError>>) {
        on_common::log_t!(LogType::WSC; "run", "generation", self.generation);
            ...
            .build()
        {
            Ok(runtime) => runtime,
            Err(error) => {
                on_common::log_e!(LogType::WSC; "run", "stage|error", "runtime_build", on_common::log::summary::error(&error));
                let _ = ready_tx.send(Err(NetError::RuntimeError));
                return;
            }
        };
        on_common::log_s!(LogType::WSC; "run", "state", "runtime_ready");
        if ready_tx.send(Ok(())).is_err() {
            on_common::log_s!(LogType::WSC; "run", "state", "ready_receiver_dropped");
        }
        runtime.block_on(self.run_async());
        on_common::log_s!(LogType::WSC; "run", "state", "worker_exited");
    }
    ...
    async fn run_async(mut self) {
        on_common::log_t!(LogType::WSC; "run_async", "generation", self.generation);
                ...
                tokio::select! {
                    ...
                    result = client.start() => {
                        if let Err(error) = result {
                            on_common::log_e!(LogType::WSC; "network_monitor_start", "error", format!("{error:?}"));
                        }
                    }
                }
            });
            ...
            tokio::select! {
                ...
                command = self.command_rx.recv() => {
                    match command {
                        ...
                        None => {
                            on_common::log_s!(LogType::WSC; "run_async", "state", "command_channel_closed");
                            should_exit = true;
                        },
                    }
                }
        ...
        on_common::log_s!(LogType::WSC; "run_async", "state|generation", "shutdown_cleanup_complete", self.generation);
        self.clear_listeners();
        if let Some(client) = self.net_status_client.take() {
            if let Err(error) = client.destroy().await {
                on_common::log_e!(LogType::WSC; "network_monitor_destroy", "error", format!("{error:?}"));
            }
        }
        timer_cancel.cancel();
        if let Err(error) = registration_timer.await {
            on_common::log_e!(LogType::WSC; "registration_timer_join", "error", format!("{error:?}"));
        }
        ...
        match command {
            ClientCommand::ConnectWithContext {
                ...
                if let Err(error) = admission {
                    if let Err(delivery_error) = session.terminate(
                        ...
                        on_common::log_e!(LogType::WSC; "connect_with_context", "stage|error", "admission_event", format!("{delivery_error:?}"));
                    }
            ...
            ClientCommand::Connect {
                ...
                if matches!(
                    ...
                    on_common::log_e!(LogType::WSC; "handle_command", "command|error", "connect", "ConnectionExists");
                    if reply.send(Err(NetError::ConnectionExists)).is_err() {
                        on_common::log_s!(LogType::WSC; "handle_command", "state", "command_reply_receiver_dropped");
                    }
                    return false;
                }
                if status == ConnectionStatus::Closed {
                    on_common::log_e!(LogType::WSC; "handle_command", "command|error", "connect", "EngineDropped");
                ...
                if !options.reconnect.is_valid()
                    || (self.network.network_status_policy()
                        ...
                        && options.reconnect.max_elapsed.is_none())
                {
                    on_common::log_e!(LogType::WSC; "handle_command", "command|error", "connect", "ConfigError");
                    if reply.send(Err(NetError::ConfigError)).is_err() {
                        on_common::log_s!(LogType::WSC; "handle_command", "state", "command_reply_receiver_dropped");
                    }
                    return false;
                }
                on_common::log_s!(LogType::WSC; "handle_command", "command|url", "connect_accepted", on_common::log::summary::url(&url));
                self.connect_target = Some(ConnectTarget {
                    url,
                    options,
                    context: None,
            ...
            ClientCommand::Disconnect { reply } => {
                on_common::log_s!(LogType::WSC; "handle_command", "command|generation", "disconnect", self.generation);
            ...
            ClientCommand::NetworkAvailable => {
                on_common::log_s!(LogType::WSC; "handle_command", "command|generation", "network_available", self.generation);
            ...
            ClientCommand::Shutdown { reply } => {
                on_common::log_s!(LogType::WSC; "handle_command", "command|generation", "shutdown", self.generation);
                self.task_observers.close();
                self.send_admission
                    .end_session_with_cause(WebSocketTaskEndCause::Shutdown);
                self.set_status(ConnectionStatus::Closing).await;
                ...
                *terminal_response_deadline = self.response_dispatch_grace_deadline();
                ...
                if let Some(deadline) = Instant::now().checked_add(self.config.close_timeout) {
                    ...
                    if tokio::time::timeout_at(deadline, delivered).await.is_err() {
                        on_common::log_s!(LogType::WSC; "handle_command", "state", "closed_status_delivery_timed_out");
                    }
                } else {
    ...
    async fn handle_io_event(&mut self, event: IoEvent) {
        ...
        match event {
            ...
            IoEvent::ConnectSucceeded {
                ...
                on_common::log_s!(LogType::WSC; "handle_io_event", "event|generation", "connect_succeeded", generation);
                if self.shutdown.is_cancelled()
                ...
                {
                    on_common::log_s!(LogType::WSC; "handle_io_event", "state|event_generation|current_generation", "stale_connection_event_ignored", generation, self.generation);
                ...
                if let Some(attempt_id) = context_attempt_id {
                    ...
                    if let Err(error) = result {
                        on_common::log_e!(LogType::WSC; "handle_io_event", "stage|error", "publish_established", format!("{error:?}"));
                        if let Some(session) = self.context_session() {
                            session.request_cancel();
                        }
                        if let Some(accepted) = accepted {
                ...
                if let Some(attempt_id) = context_attempt_id {
                    if let Some(session) = self.context_session() {
                        if let Err(error) = session.commit_established(generation, attempt_id) {
                            on_common::log_e!(LogType::WSC; "handle_io_event", "stage|error", "commit_established", format!("{error:?}"));
                            session.request_cancel();
                        }
                    }
                }
                if let Some(reply) = self.connect_reply.take() {
                    if reply.send(Ok(())).is_err() {
                        on_common::log_s!(LogType::WSC; "handle_io_event", "state", "command_reply_receiver_dropped");
                    }
                }
                if let Some(accepted) = accepted {
            ...
            IoEvent::ConnectFailed {
                ...
                on_common::log_e!(LogType::WSC; "handle_io_event", "event|generation|error|http_status", "connect_failed", generation, format!("{error:?}"), http_status);
            ...
            IoEvent::CallbackDispatchFailed { generation } => {
                ...
                {
                    on_common::log_s!(LogType::WSC; "handle_io_event", "state|event_generation|current_generation", "stale_callback_failure_ignored", generation, self.generation);
                    return;
                }
            ...
            | IoEvent::WriteEnded { generation, error } => {
                on_common::log_s!(LogType::WSC; "handle_io_event", "event|generation|error", "io_ended", generation, format!("{error:?}"));
                if generation != self.generation
                    || self.current_status() != ConnectionStatus::Connected
                {
                    on_common::log_s!(LogType::WSC; "handle_io_event", "state|event_generation|current_generation", "stale_io_event_ignored", generation, self.generation);
                    return;
                }
    ...
    async fn connection_lost_at_stage(
        ...
        if let Some(session) = self.context_session() {
            if let Err(event_error) = session.connection_terminated(
                ...
                on_common::log_e!(LogType::WSC; "handle_io_event", "stage|error", "publish_connection_terminated", format!("{event_error:?}"));
                session.request_cancel();
            }
        }
    ...
    fn acknowledge_context_event(
        ...
        if let Err(error) = result {
            on_common::log_e!(LogType::WSC; "acknowledge_context_event", "error", format!("{error:?}"));
    ...
    fn finish_context_session(
        ...
        if let Some(session) = self.context_session() {
            if let Err(error) = session.terminate(reason, failure) {
                on_common::log_e!(LogType::WSC; "finish_context_session", "error", format!("{error:?}"));
            }
        }
    }
    ...
    async fn start_connection(&mut self, reconnecting: bool) {
        on_common::log_t!(LogType::WSC; "start_connection", "reconnecting|generation", reconnecting, self.generation);
        if self.shutdown.is_cancelled()
            || self
                .context_session()
                .is_some_and(|session| session.cancel_token().is_cancelled())
        ...
        let Some(target) = self.connect_target.clone() else {
            on_common::log_s!(LogType::WSC; "start_connection", "state", "no_connect_target");
            return;
        };
        ...
        on_common::log_s!(LogType::WSC; "start_connection", "state|generation|reconnecting", "connection_cycle_started", generation, reconnecting);
    ...
    fn cancel_connect(&mut self, error: NetError) {
        on_common::log_t!(LogType::WSC; "cancel_connect", "error|generation", format!("{error:?}"), self.generation);
        if let Some(cancel) = self.connect_cancel.take() {
            on_common::log_s!(LogType::WSC; "cancel_connect", "state|generation", "connection_attempt_cancelled", self.generation);
            cancel.cancel();
        }
        ...
        if let Some(reply) = self.connect_reply.take() {
            if reply.send(Err(error)).is_err() {
                on_common::log_s!(LogType::WSC; "cancel_connect", "state", "command_reply_receiver_dropped");
            }
        }
    }
    ...
    async fn stop_active_io(&mut self, graceful: bool) {
        on_common::log_t!(LogType::WSC; "stop_active_io", "graceful|generation", graceful, self.generation);
        let Some(active) = self.active_io.take() else {
            on_common::log_s!(LogType::WSC; "stop_active_io", "state", "no_active_io");
            return;
        };
        let ActiveIo {
            generation,
        ...
        if graceful && generation == self.generation {
            ...
            match graceful_result {
                Ok(Ok(())) => {
                    on_common::log_s!(LogType::WSC; "stop_active_io", "state|generation", "close_write_acknowledged", generation)
                }
                Ok(Err(error)) => {
                    on_common::log_s!(LogType::WSC; "stop_active_io", "state|generation|error", "close_channel_unavailable", generation, format!("{error:?}"))
                }
                Err(_) => {
                    on_common::log_s!(LogType::WSC; "stop_active_io", "state|generation", "graceful_close_timed_out", generation)
                }
            }
        }
        ...
        let joined = tokio::time::timeout_at(deadline, async {
            ...
            if let Err(error) = read_result {
                on_common::log_e!(LogType::WSC; "stop_active_io", "task|error", "read", on_common::log::summary::error(&error));
            }
            if let Err(error) = write_result {
                on_common::log_e!(LogType::WSC; "stop_active_io", "task|error", "write", on_common::log::summary::error(&error));
            }
        })
        .await;
        if joined.is_err() {
            on_common::log_s!(LogType::WSC; "stop_active_io", "state|generation", "cooperative_stop_timed_out_aborting_io", generation);
            read_handle.abort();
            write_handle.abort();
    ...
    fn current_status(&self) -> ConnectionStatus {
        on_common::log_t!(LogType::WSC; "current_status");
        self.state.read().map(|state| *state).unwrap_or_else(|_| {
            on_common::log_e!(LogType::WSC; "current_status", "error", "state_lock_poisoned");
            ConnectionStatus::Closed
        })
    }
    ...
    fn set_last_connection_error(&self, error: Option<NetError>) {
        on_common::log_t!(LogType::WSC; "set_last_connection_error", "error", format!("{error:?}"));
        if let Ok(mut current) = self.last_connection_error.write() {
            *current = error;
        } else {
            on_common::log_e!(LogType::WSC; "set_last_connection_error", "error", "snapshot_lock_poisoned");
        }
    }

    /// 更新最近一次终态 HTTP 握手状态；锁中毒时不阻塞生命周期。
    fn set_last_handshake_http_status(&self, status: Option<u16>) {
        on_common::log_t!(LogType::WSC; "set_last_handshake_http_status", "status", status);
        if let Ok(mut current) = self.last_handshake_http_status.write() {
            *current = status;
        } else {
            on_common::log_e!(LogType::WSC; "set_last_handshake_http_status", "error", "snapshot_lock_poisoned");
        }
    }

    /// Breaks listener/client reference cycles once the client reaches terminal shutdown.
    fn clear_listeners(&self) {
        on_common::log_t!(LogType::WSC; "clear_listeners");
        if let Ok(mut listener) = self.listeners.data.write() {
            listener.take();
        } else {
            on_common::log_e!(LogType::WSC; "clear_listeners", "error", "data_listener_lock_poisoned");
        }
        let status_listener = if let Ok(mut listener) = self.listeners.status.write() {
            listener.take()
        } else {
            on_common::log_e!(LogType::WSC; "clear_listeners", "error", "status_listener_lock_poisoned");
            None
        };
        ...
        on_common::log_s!(LogType::WSC; "clear_listeners", "state", "terminal_listener_cleanup_finished");
        let subscription = self
            .listeners
            .log
            .lock()
            .unwrap_or_else(|poisoned| {
                on_common::log_e!(LogType::WSC; "clear_listeners", "error", "log_listener_lock_poisoned_recovered");
                poisoned.into_inner()
            })
            .take();
        drop(subscription);
    ...
    fn response_dispatch_grace_deadline(&self) -> Option<Instant> {
        on_common::log_t!(LogType::WSC; "response_dispatch_grace_deadline");
    ...
    async fn drain_data_callbacks_until(&self, deadline: Option<Instant>) {
        on_common::log_t!(LogType::WSC; "drain_data_callbacks_until", "deadline_present", deadline.is_some());
        ...
        if tokio::time::timeout_at(deadline, drain).await.is_err() {
            on_common::log_s!(LogType::WSC; "drain_data_callbacks_until", "state", "response_grace_expired");
        } else {
            on_common::log_s!(LogType::WSC; "drain_data_callbacks_until", "state", "data_lane_drain_finished");
        }
    }
    ...
    async fn set_status(&self, status: ConnectionStatus) {
        on_common::log_t!(LogType::WSC; "set_status", "status|generation", format!("{status:?}"), self.generation);
        let (changed, previous) = if let Ok(mut current) = self.state.write() {
            ...
            if previous == status {
                ...
                on_common::log_e!(LogType::WSC; "set_status", "error|from|to", "invalid_status_transition", format!("{previous:?}"), format!("{status:?}"));
                (false, Some(previous))
            } else {
                *current = status;
                (true, Some(previous))
            ...
            on_common::log_e!(LogType::WSC; "set_status", "error", "state_lock_poisoned");
            (false, None)
        };
        on_common::log_s!(LogType::WSC; "set_status", "from|to|changed|generation", format!("{previous:?}"), format!("{status:?}"), changed, self.generation);
        if changed {
            match self.status_callback_tx.try_send(CallbackEvent::Status {
                ...
                Ok(()) => {
                    on_common::log_s!(LogType::WSC; "set_status", "state", "status_notification_queued")
                }
                Err(mpsc::error::TrySendError::Full(_)) => {
                    on_common::log_s!(LogType::WSC; "set_status", "state", "intermediate_status_notification_coalesced")
                }
                Err(mpsc::error::TrySendError::Closed(_)) => {
                    on_common::log_s!(LogType::WSC; "set_status", "state", "status_callback_lane_closed")
                }
            }
        }
    }
    ...
    async fn set_status_and_wait(&self, status: ConnectionStatus) {
        on_common::log_t!(LogType::WSC; "set_status_and_wait", "status|generation", format!("{status:?}"), self.generation);
        let (changed, previous) = if let Ok(mut current) = self.state.write() {
            let previous = *current;
            if previous == status {
                (false, Some(previous))
            } else if !connection_status_transition_allowed(previous, status) {
                on_common::log_e!(LogType::WSC; "set_status_and_wait", "error|from|to", "invalid_status_transition", format!("{previous:?}"), format!("{status:?}"));
                (false, Some(previous))
            } else {
                *current = status;
                (true, Some(previous))
            ...
            on_common::log_e!(LogType::WSC; "set_status_and_wait", "error", "state_lock_poisoned");
            (false, None)
        };
        on_common::log_s!(LogType::WSC; "set_status_and_wait", "from|to|changed|generation", format!("{previous:?}"), format!("{status:?}"), changed, self.generation);
        if !changed {
            return;
        }
        let (delivered_tx, delivered_rx) = oneshot::channel();
...
fn connection_status_transition_allowed(from: ConnectionStatus, to: ConnectionStatus) -> bool {
    on_common::log_t!(LogType::WSC; "connection_status_transition_allowed", "from|to", format!("{from:?}"), format!("{to:?}"));
...
{
    on_common::log_t!(LogType::WSC; "data_callback_loop", "max_concurrency", max_concurrency);
    ...
    loop {
        ...
        match event {
            CallbackEvent::Data {
                ...
                on_common::log_s!(LogType::WSC; "data_callback_loop", "state|listener_present", "data_event_dequeued", listener.is_some());
                if let Some(listener) = listener {
                    ...
                    match dispatch(Box::new(move || listener(response))) {
                        ...
                        Err(error) => {
                            drop(byte_permit);
                            on_common::log_e!(LogType::WSC; "data_callback_loop", "stage|generation|error", "callback_thread_spawn_failed", generation, on_common::log::summary::error(&error));
                            ...
                            tokio::select! {
                                biased;
                                _ = shutdown.cancelled() => {
                                    on_common::log_s!(LogType::WSC; "data_callback_loop", "state|generation", "callback_failure_after_shutdown", generation);
                                }
                                sent = io_event_tx.send(IoEvent::CallbackDispatchFailed { generation }) => {
                                    if sent.is_err() {
                                        on_common::log_e!(LogType::WSC; "data_callback_loop", "stage|generation", "callback_failure_receiver_closed", generation);
                                    }
                                }
                            }
                        }
            ...
            CallbackEvent::Status { delivered, .. } => {
                on_common::log_e!(LogType::WSC; "data_callback_loop", "error", "status_event_in_data_lane");
...
async fn status_callback_loop(
    ...
    on_common::log_t!(LogType::WSC; "status_callback_loop", "listeners|state|queued_events", "shared", "shared", callback_rx.len());
    while let Some(event) = callback_rx.recv().await {
        match event {
            CallbackEvent::Status { status, delivered } => {
                loop {
                    let registration = match listeners.status.read() {
                        ...
                        Err(_) => {
                            on_common::log_e!(LogType::WSC; "status_callback_loop", "error", "status_listener_lock_poisoned");
                            None
                        }
                    };
                    ...
                    let status = match state.read() {
                        Ok(current) => *current,
                        Err(_) => {
                            on_common::log_e!(LogType::WSC; "status_callback_loop", "error", "state_lock_poisoned");
                            status
                        }
                    };
                    on_common::log_s!(LogType::WSC; "status_callback_loop", "status|listener_present", format!("{status:?}"), true);
            ...
            CallbackEvent::Data { .. } => {
                on_common::log_e!(LogType::WSC; "status_callback_loop", "error", "data_event_in_status_lane");
            }
            CallbackEvent::DataDrain { delivered } => {
                on_common::log_e!(LogType::WSC; "status_callback_loop", "error", "drain_event_in_status_lane");
                let _ = delivered.send(());
            }
        }
    }
...
async fn publish_legacy_failure(
    ...
    if event_tx
        ...
        .is_err()
    {
        on_common::log_e!(LogType::WSC; "connect_with_retry", "error", "worker_event_receiver_closed");
    }
}
...
fn configure_tcp_socket(
    ...
    on_common::log_t!(LogType::WSC; "configure_tcp_socket", "send_buffer_size|keepalive_enabled", send_buffer_size, config.is_some());
    ...
    if let Some(send_buffer_size) = send_buffer_size {
        if let Err(error) = socket.set_send_buffer_size(send_buffer_size) {
            on_common::log_e!(LogType::WSC; "configure_tcp_socket", "option|requested_size|error", "send_buffer_size", send_buffer_size, on_common::log::summary::error(&error));
        } else {
            on_common::log_s!(LogType::WSC; "configure_tcp_socket", "option|state", "send_buffer_size", "applied");
        }
    }
    if let Some(config) = config {
        ...
        if let Err(error) = socket.set_tcp_keepalive(&keepalive) {
            on_common::log_e!(LogType::WSC; "configure_tcp_socket", "option|error", "tcp_keepalive", on_common::log::summary::error(&error));
        } else {
            on_common::log_s!(LogType::WSC; "configure_tcp_socket", "option|state", "tcp_keepalive", "applied");
        }
    }
}
...
async fn build_connect_request(
    ...
    on_common::log_t!(LogType::WSC; "build_connect_request", "url|header_count|provider_present|deadline_elapsed", on_common::log::summary::url(&target.url), target.options.headers.len(), target.options.header_provider.is_some(), Instant::now() >= deadline);
    let mut request = target
        .url
        .as_str()
        .into_client_request()
        .map_err(|error| {
            on_common::log_e!(LogType::WSC; "build_connect_request", "stage|error", "url_request", on_common::log::summary::error(&error));
    ...
    if let Some(provider) = target.options.header_provider.as_ref() {
        ...
        let dynamic_headers =
            tokio::time::timeout_at(deadline, run_header_provider_detached(Arc::clone(provider), permit))
                .await
                .map_err(|_| {
                    on_common::log_e!(LogType::WSC; "build_connect_request", "stage|error", "header_provider", "timeout");
                    ConnectAttemptError::ProviderTimedOut
                })?
    ...
    for (name, value) in headers {
        let name = HeaderName::from_bytes(name.as_bytes())
            .map_err(|error| {
                on_common::log_e!(LogType::WSC; "build_connect_request", "stage|error", "header_validation", on_common::log::summary::error(&error));
    ...
    on_common::log_s!(LogType::WSC; "build_connect_request", "state|header_count", "request_built", request.headers().len());
    Ok(request)
}
...
async fn run_header_provider_detached(
    ...
    on_common::log_t!(LogType::WSC; "run_header_provider_detached", "provider", "registered");
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let result = catch_unwind(AssertUnwindSafe(|| provider()))
                .unwrap_or_else(|_| {
                    on_common::log_e!(LogType::WSC; "run_header_provider_detached", "error", "header_provider_panicked");
                    Err(NetError::TaskInterruptionError)
                });
            if let Err(error) = &result {
                on_common::log_e!(LogType::WSC; "run_header_provider_detached", "stage|error", "header_provider", format!("{error:?}"));
            }
            if result_tx.send(result).is_err() {
                on_common::log_s!(LogType::WSC; "run_header_provider_detached", "state", "result_receiver_dropped");
            }
        })
        .map_err(|error| {
            on_common::log_e!(LogType::WSC; "run_header_provider_detached", "stage|error", "thread_spawn", on_common::log::summary::error(&error));
            NetError::TaskInterruptionError
        })?;
    result_rx
        .await
        .unwrap_or_else(|_| {
            on_common::log_e!(LogType::WSC; "run_header_provider_detached", "error", "provider_result_channel_closed");
            Err(NetError::TaskInterruptionError)
        })
}
...
fn retry_allowed(
    ...
    on_common::log_t!(LogType::WSC; "retry_allowed", "attempt|elapsed_seconds|max_retries|enabled", attempt, elapsed.as_secs_f64(), policy.max_retries, policy.enabled);
...
fn full_jitter_delay(
    ...
    on_common::log_t!(LogType::WSC; "full_jitter_delay", "attempt|max_delay_seconds", attempt, policy.max_delay.as_secs_f64());
...
fn next_jitter_random() -> u64 {
    on_common::log_t!(LogType::WSC; "next_jitter_random");
    ...
    impl Drop for CallbackRelease {
        fn drop(&mut self) {
            ...
            let mut state = match lock.lock() {
                Ok(state) => state,
                Err(error) => {
                    on_common::log_e!(LogType::WSC; "test_callback", "error", format!("callback state lock poisoned during test cleanup: {error}"));
                    error.into_inner()
                }
            };
    ...
    fn concurrency_listener(
        ...
        Arc::new(move |response| {
            ...
            if results.send(result).is_err() {
                on_common::log_e!(LogType::WSC; "test_callback", "error", format!("callback result receiver closed"));
            }
        })
    }
            ...
            Box::new(move |record| {
                if record.tag == "ON_WSC-set_status-S" {
                    ...
                    if relevant && log_tx.send(result).is_err() {
                        on_common::log_e!(LogType::WSC; "test_callback", "error", format!("state transition log receiver closed"));
                    }
                }
            }),
            &[LogType::WSC],
        ...
        let listener: crate::module::ws_client::listener_store::DataListener = Arc::new(
            move |response| {
                ...
                if handoff_tx.send(result).is_err() {
                    on_common::log_e!(LogType::WSC; "test_callback", "error", format!("response handoff result receiver closed"));
                }
            },
        );
        *worker
            ...
            Box::new(move |record| {
                if matches!(
                    ...
                    if log_tx.send(result).is_err() {
                        on_common::log_e!(LogType::WSC; "test_callback", "error", format!("status error log receiver closed"));
                    }
                }
            }),
            &[LogType::WSC],
        ...
        let _subscription = Logger::register_log_listener_with_capacity(
            Box::new(move |record| {
                if matches!(
                    ...
                    if log_tx.send(result).is_err() {
                        on_common::log_e!(LogType::WSC; "test_callback", "error", format!("callback error log receiver closed"));
                    }
                }
            }),
            &[LogType::WSC],
        ...
        data_tx.send(CallbackEvent::Data {
            listener: Some(Arc::new(move |_response| {
                if data_seen_tx.send(()).is_err() {
                    on_common::log_e!(LogType::WSC; "test_callback", "error", format!("data callback observation receiver closed"));
                }
            })),
            ...
            crate::module::ws_client::listener_store::StatusRegistration::new(Arc::new(
                move |status| {
                    if status_seen_tx.send(status).is_err() {
                        on_common::log_e!(LogType::WSC; "test_callback", "error", format!("status callback observation receiver closed"));
                    }
                },
            )),
        );

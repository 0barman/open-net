...
impl PendingRequestCompletion {
    pub(in crate::api::wsc) fn new(
        ...
        on_common::log_t!(LogType::WSC; "new", "request_id|receiver", request_id, "oneshot::Receiver");
        Self {
            request_id,
            receiver,
        }
    ...
    pub async fn wait(self) -> Result<(), NetError> {
        on_common::log_t!(LogType::WSC; "wait");
        let result = self.receiver.await.unwrap_or(Err(NetError::EngineDropped));
        match result {
            Ok(()) => {
                on_common::log_s!(LogType::WSC; "wait", "request_id|state", self.request_id, "response_claimed")
            }
            Err(NetError::Cancelled) => {
                on_common::log_s!(LogType::WSC; "wait", "request_id|state", self.request_id, "cancelled")
            }
            Err(error) => {
                on_common::log_e!(LogType::WSC; "wait", "request_id|error", self.request_id, format!("{:?}", error))
            }
        }
        result
    }

...
impl PriorityWriteQueue {
    ...
    pub(crate) fn new(max_tasks: usize, max_bytes: usize) -> Result<Arc<Self>, NetError> {
        crate::log_t!(LogType::WSC; "new", "max_tasks|max_bytes", max_tasks, max_bytes);
        if max_tasks == 0
        ...
        {
            crate::log_e!(LogType::WSC; "new", "error", "ConfigError");
    ...
    fn is_closed(&self) -> bool {
        crate::log_t!(LogType::WSC; "is_closed");
        self.state
            .lock()
            .map(|state| state.closed)
            .unwrap_or_else(|_| {
                crate::log_e!(LogType::WSC; "is_closed", "error", "queue_lock_poisoned");
                true
            })
    }
    ...
    pub(crate) async fn enqueue_mode(
        ...
        let acquire = async {
            let slot = tokio::select! {
                ...
                _ = shutdown.cancelled() => {
                    crate::log_s!(LogType::WSC; "enqueue", "stage|state", "task_capacity", "cancelled");
                    return Err(NetError::Cancelled);
                },
                _ = admission_cancel.cancelled() => {
                    crate::log_e!(LogType::WSC; "enqueue", "stage|error", "task_capacity", "ConnectionClosed");
                ...
                permit = Arc::clone(&self.task_slots).acquire_owned() => {
                    permit.map_err(|_| {
                        crate::log_e!(LogType::WSC; "enqueue", "stage|error", "task_capacity", "QueueClosed");
            ...
            let byte = tokio::select! {
                _ = dispatch_cancel.cancelled() => { return Err(NetError::Cancelled); },
                _ = shutdown.cancelled() => {
                    crate::log_s!(LogType::WSC; "enqueue", "stage|state", "byte_capacity", "cancelled");
                    return Err(NetError::Cancelled);
                },
                _ = admission_cancel.cancelled() => {
                    crate::log_e!(LogType::WSC; "enqueue", "stage|error", "byte_capacity", "ConnectionClosed");
                    return Err(if dispatch_cancel.is_cancelled() {
                        NetError::Cancelled
                    } else { NetError::ConnectionClosed });
                },
                permit = Arc::clone(&self.byte_slots).acquire_many_owned(bytes as u32) => {
                    permit.map_err(|_| {
                        crate::log_e!(LogType::WSC; "enqueue", "stage|error", "byte_capacity", "QueueClosed");
                        NetError::QueueClosed
                    })?
                }
            };
        ...
        let (slot_permit, byte_permit) = match config.enqueue_timeout {
            Some(duration) => {
                let deadline = tokio::time::Instant::now()
                    ...
                    .ok_or_else(|| {
                        crate::log_e!(LogType::WSC; "enqueue", "stage|error", "enqueue_deadline", "ConfigError");
                        NetError::ConfigError
                    })?;
                tokio::time::timeout_at(deadline, acquire)
                    .await
                    .map_err(|_| {
                        crate::log_e!(LogType::WSC; "enqueue", "stage|error", "capacity", "TimeoutError");
        ...
        if shutdown.is_cancelled() {
            crate::log_s!(LogType::WSC; "enqueue", "error", "Cancelled");
            return Err(NetError::Cancelled);
        }
        if admission_cancel.is_cancelled() {
            crate::log_e!(LogType::WSC; "enqueue", "error", "ConnectionClosed");
        ...
        if let Err(request) = self.push_request(request, prepared) {
            ...
            crate::log_e!(LogType::WSC; "enqueue", "error", format!("{:?}", error));
            return Err(error);
        }
        Ok(result_rx)
    }
    ...
    pub(crate) fn try_enqueue_mode(
        ...
        crate::log_t!(LogType::WSC; "try_enqueue", "uuid|pending_token|message_size|message_type|config|shutdown|dispatch_cancel|dispatch_phase|admission_cancel", uuid, pending_token, message_size, if message.is_text() { "Text" } else if message.is_binary() { "Binary" } else { "Control" }, format!("{:?}", config), shutdown.is_cancelled(), dispatch_cancel.is_cancelled(), "DispatchPhase", admission_cancel.is_cancelled());
        let bytes = message_size.max(1);
        if bytes > self.max_bytes as usize {
            crate::log_e!(LogType::WSC; "try_enqueue", "error", "QueueItemTooLarge");
            return Err(NetError::QueueItemTooLarge);
        }
        if self.is_closed() {
            crate::log_e!(LogType::WSC; "try_enqueue", "error", "QueueClosed");
            return Err(NetError::QueueClosed);
        }
        ...
        if shutdown.is_cancelled() {
            crate::log_s!(LogType::WSC; "try_enqueue", "error", "Cancelled");
            return Err(NetError::Cancelled);
        }
        if admission_cancel.is_cancelled() {
            crate::log_e!(LogType::WSC; "try_enqueue", "error", "ConnectionClosed");
            return Err(NetError::ConnectionClosed);
        }
        if self.is_closed() {
            crate::log_e!(LogType::WSC; "try_enqueue", "error", "QueueClosed");
        ...
        if let Err(request) = self.push_request(request, prepared) {
            ...
            request.complete(Err(error));
            crate::log_e!(LogType::WSC; "try_enqueue", "error", format!("{:?}", error));
            return Err(error);
        }
        Ok(result_rx)
    }
    ...
    fn push_request(
        ...
        crate::log_t!(LogType::WSC; "push_existing", "uuid|pending_token|sequence|attempt|priority", request.uuid, request.pending_token, request.sequence, request.attempt, format!("{:?}", request.config.priority));
        let Ok(mut state) = self.state.lock() else {
            crate::log_e!(LogType::WSC; "push_existing", "error", "queue_lock_poisoned");
        ...
        if state.closed
            ...
            || request
                ...
                .is_some_and(CancellationToken::is_cancelled)
        {
            crate::log_s!(LogType::WSC; "push_existing", "uuid|sequence|state|closed", request.uuid, request.sequence, "enqueue_rejected", state.closed);
            return Err(Box::new(request));
        }
        ...
        crate::log_s!(LogType::WSC; "push_existing", "sequence|attempt|state|queued_count", sequence, attempt, "Queued", queued_count);
        Ok(())
    }
    ...
    pub(crate) fn cancel_queued_with_error(
        ...
        crate::log_t!(LogType::WSC; "cancel_queued", "dispatch_cancel", dispatch_cancel.is_cancelled());
        let Ok(mut state) = self.state.lock() else {
            crate::log_e!(LogType::WSC; "cancel_queued", "error", "queue_lock_poisoned");
            return false;
        };
        ...
        if let Some(request) = removed {
            crate::log_s!(LogType::WSC; "cancel_queued", "uuid|sequence|state", request.uuid, request.sequence, "cancelled");
            request.complete(Err(error));
            true
        } else {
            false
            ...
            let closed = match self.state.lock() {
                Ok(mut state) => {
                    if let Some(request) = state.heap.pop() {
                        let queued_count = state.heap.len();
                        drop(state);
                        crate::log_s!(LogType::WSC; "next", "uuid|sequence|queued_count|state", request.uuid, request.sequence, queued_count, "dequeued");
                        return Some(request);
                    }
                    state.closed
                }
                Err(_) => {
                    crate::log_e!(LogType::WSC; "next", "error", "queue_lock_poisoned");
                    true
                }
            };
            if closed {
                crate::log_s!(LogType::WSC; "next", "state", "closed_and_empty");
                return None;
            }
            notified.await;
        }
    ...
    pub(crate) fn try_next(&self) -> Option<QueuedRequest> {
        crate::log_t!(LogType::WSC; "try_next");
        let request = self
            .state
            .lock()
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "try_next", "error", "queue_lock_poisoned");
            })
            .ok()
            .and_then(|mut state| state.heap.pop());
        if let Some(request) = &request {
            crate::log_s!(LogType::WSC; "try_next", "uuid|sequence|state", request.uuid, request.sequence, "dequeued");
        }
        request
    }
    ...
    pub(crate) fn drain(&self) -> Vec<QueuedRequest> {
        ...
        let drained: Vec<_> = self
            ...
            .unwrap_or_else(|_| {
                crate::log_e!(LogType::WSC; "drain", "error", "queue_lock_poisoned");
                Vec::new()
            });
        crate::log_s!(LogType::WSC; "drain", "drained_count", drained.len());
        drained
    }
    ...
    pub(crate) fn drain_rejected_on_disconnect(&self) -> Vec<QueuedRequest> {
        crate::log_t!(LogType::WSC; "drain_rejected_on_disconnect");
        let Ok(mut state) = self.state.lock() else {
            crate::log_e!(LogType::WSC; "drain_rejected_on_disconnect", "error", "queue_lock_poisoned");
            return Vec::new();
        };
        ...
        drop(state);
        crate::log_s!(LogType::WSC; "drain_rejected_on_disconnect", "retained_count|rejected_count", retained_count, rejected.len());
        rejected
    }
    ...
    pub(crate) fn close(&self) {
        crate::log_t!(LogType::WSC; "close");
        if let Ok(mut state) = self.state.lock() {
            state.closed = true;
        } else {
            crate::log_e!(LogType::WSC; "close", "error", "queue_lock_poisoned");
        }
        self.task_slots.close();
        self.byte_slots.close();
        self.notify.notify_waiters();
        crate::log_s!(LogType::WSC; "close", "state", "queue_closed");
    }
}

fn map_try_acquire_error(error: TryAcquireError) -> NetError {
    crate::log_t!(LogType::WSC; "map_try_acquire_error", "error", format!("{:?}", error));
    crate::log_e!(LogType::WSC; "map_try_acquire_error", "error", format!("{:?}", error));
    match error {

...
impl ObservationScope {
    ...
    fn lock_state(&self) -> std::sync::MutexGuard<'_, ObservationScopeState> {
        match self.state.lock() {
            Ok(state) => state,
            Err(poisoned) => {
                crate::log_e!(LogType::WSC; "task_scope", "error", "lock_poisoned_recovered");
                poisoned.into_inner()
            }
        }
    }
...
impl SendAdmission {
    ...
    pub(crate) fn begin_session_with_scope(
        ...
        crate::log_t!(LogType::WSC; "begin_session");
        ...
        drop(inner);
        crate::log_s!(LogType::WSC; "begin_session", "state", "session_started_connection_closed");
    }
    ...
    pub(crate) fn connection_succeeded_with_network_epoch(&self, loss_epoch: u64) {
        crate::log_t!(LogType::WSC; "connection_succeeded");
        let mut inner = self.lock_inner();
        inner.connection.cancel();
        ...
        drop(inner);
        crate::log_s!(LogType::WSC; "connection_succeeded", "state", "connection_admission_open");
    }

    /// Invalidates requests admitted only for the current physical connection.
    pub(crate) fn connection_ended(&self) {
        crate::log_t!(LogType::WSC; "connection_ended");
        ...
        crate::log_s!(LogType::WSC; "connection_ended", "state", "connection_admission_cancelled");
    }
    ...
    pub(crate) fn end_session_with_cause(&self, cause: WebSocketTaskEndCause) {
        crate::log_t!(LogType::WSC; "end_session");
        let inner = self.lock_inner();
        ...
        crate::log_s!(LogType::WSC; "end_session", "state", "session_admission_cancelled");
    }
    ...
    pub(crate) fn lease_observed(&self, policy: DisconnectedTaskPolicy) -> SendLease {
        crate::log_t!(LogType::WSC; "lease", "policy", format!("{:?}", policy));
        let inner = self.lock_inner();
...
fn cancelled_token() -> CancellationToken {
    crate::log_t!(LogType::WSC; "cancelled_token");

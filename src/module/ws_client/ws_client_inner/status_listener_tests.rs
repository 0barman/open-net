...
fn checked_listener<T: Send + 'static>(
    ...
    Box::new(move |status| {
        if completed.send(callback(status)).is_err() {
            crate::log_e!(LogType::WSC; "test_status_callback", "error", "callback result receiver closed");
        }
    })
}
    ...
    let thread = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if returned.send(()).is_err() {
                crate::log_e!(LogType::WSC; "test_status_registration", "error", "registration result receiver closed");
            }
        })?;
...
impl Drop for ReentrantListenerDrop {
    fn drop(&mut self) {
        ...
        if self.completed.send(result).is_err() {
            crate::log_e!(LogType::WSC; "test_status_listener_drop", "error", "drop result receiver closed");
        }
    }
}
...
impl Drop for CapturedClientRelease {
    fn drop(&mut self) {
        ...
        if self.completed.send(()).is_err() {
            crate::log_e!(LogType::WSC; "test_status_client_capture_drop", "error", "drop result receiver closed");
        }
    }
}

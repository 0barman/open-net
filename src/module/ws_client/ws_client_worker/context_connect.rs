...
async fn run_provider(
    ...
    std::thread::Builder::new().name("open-net-ws-context-provider".to_string()).spawn(move || {
        ...
        let result = match catch_unwind(AssertUnwindSafe(|| provider(attempt))) {
            Ok(result) => result,
            Err(_) => {
                crate::log_e!(LogType::WSC; "context_provider", "error", "provider_panicked");
                Err(NetError::TaskInterruptionError)
            }
        };
        if let Err(error) = result.as_ref() {
            crate::log_e!(LogType::WSC; "context_provider", "error", format!("{error:?}"));
        }
        let _ = result_tx.send(result);
    }).map_err(|error| {
        crate::log_e!(LogType::WSC; "context_provider", "stage|error", "thread_spawn", crate::common::log::summary::error(&error));

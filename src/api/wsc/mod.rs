...
pub(crate) fn duration_fits_instant(duration: Duration) -> bool {
    crate::log_t!(LogType::WSC; "duration_fits_instant", "duration", format!("{:?}", duration));
    let fits = Instant::now().checked_add(duration).is_some();
    if !fits {
        crate::log_e!(LogType::WSC; "duration_fits_instant", "duration|error", format!("{:?}", duration), "unrepresentable_deadline");
    }
    fits
}

...
impl RegistrationState {
    ...
    pub(crate) fn deadline(&self) -> Result<Option<Instant>, NetError> {
        self.deadline.lock().map(|value| *value).map_err(|_| {
            crate::log_e!(LogType::WSC; "registration_deadline", "error", "timing_lock_poisoned");
    ...
    pub(crate) fn finish(&self, result: Result<(), NetError>) {
        let mut selected = self.result.lock().unwrap_or_else(|poisoned| {
            crate::log_e!(LogType::WSC; "registration_finish", "error", "result_lock_poisoned_recovered");
            poisoned.into_inner()
        });
        if selected.is_none() {
    ...
    fn terminal_error(&self) -> Result<Option<NetError>, NetError> {
        self.result
            .lock()
            .map(|result| result.and_then(Result::err))
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "registration_result", "error", "result_lock_poisoned");
                NetError::InternalError
            })
    }
}
...
impl RequestRegistrationToken {
    ...
    fn allocate_from(sequence: &AtomicU64) -> Result<Self, NetError> {
        sequence
            ...
            .map(Self)
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "allocate_registration_token", "error", "registration_identity_exhausted");
                NetError::InternalError
            })
    }

...
impl PendingRequestView {
    pub(crate) fn with_capacity(max_entries: usize) -> Self {
        crate::log_t!(LogType::WSC; "with_capacity", "max_entries", max_entries);
        Self {
    ...
    pub fn get_request(&self, uuid: &str, generation: u64) -> Option<Arc<dyn WSRequestTrait>> {
        crate::log_t!(LogType::WSC; "get_request", "uuid|generation", uuid, generation);
        self.entries
            .read()
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "get_request", "error", "pending_lock_poisoned");
            })
            .ok()
            .and_then(|entries| {
    ...
    pub fn take_request(&self, uuid: &str, generation: u64) -> Option<Arc<dyn WSRequestTrait>> {
        crate::log_t!(LogType::WSC; "take_request", "uuid|generation", uuid, generation);
        let mut entries = self
            .entries
            .write()
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "take_request", "error", "pending_lock_poisoned");
            })
            .ok()?;
        ...
        if let Some(entry) = &entry {
            if entry.info.status == PendingRequestStatus::Writing && entry.deferred_write.is_none()
            {
                self.write_response_claims
                    .lock()
                    .unwrap_or_else(|poisoned| { crate::log_e!(LogType::WSC; "take_request", "error", "lock_poisoned_recovered"); poisoned.into_inner() })
                    .insert(entry.token);
            }
        }
        drop(entries);
        ...
        crate::log_s!(LogType::WSC; "take_request", "uuid|generation|claimed", uuid, generation, request.is_some());
        request
    }
    ...
    pub(in crate::api::wsc) fn take_registered_request(
        ...
        let mut entries = self.entries.write().map_err(|_| {
            crate::log_e!(LogType::WSC; "take_registered_request", "error", "pending_lock_poisoned");
        ...
        if writing {
            self.write_response_claims.lock().map_err(|_| {
                crate::log_e!(LogType::WSC; "take_registered_request", "error", "write_claim_lock_poisoned");
                NetError::InternalError
    ...
    pub(in crate::api::wsc) fn terminate_registration(
        ...
        let mut entries = self.entries.write().map_err(|_| {
            crate::log_e!(LogType::WSC; "terminate_registration", "error", "pending_lock_poisoned");
            NetError::InternalError
        })?;
    ...
    pub(crate) fn take_write_response_claim(&self, token: u64) -> bool {
        crate::log_t!(LogType::WSC; "take_write_response_claim", "token", token);
        self.write_response_claims
            .lock()
    ...
    pub fn snapshot(&self) -> Vec<PendingRequestInfo> {
        crate::log_t!(LogType::WSC; "snapshot");
        self.entries
            .read()
            .map(|entries| entries.values().map(|entry| entry.info.clone()).collect())
            .unwrap_or_else(|_| {
                crate::log_e!(LogType::WSC; "snapshot", "error", "pending_lock_poisoned");
                Vec::new()
            })
    }
    ...
    pub fn len(&self) -> usize {
        ...
        self.entries
            ...
            .unwrap_or_else(|_| {
                crate::log_e!(LogType::WSC; "len", "error", "pending_lock_poisoned");
                0
            })
    }
    ...
    pub(crate) fn reserve_timed_snapshot_observed(
        ...
        crate::log_t!(LogType::WSC; "reserve", "request_type|config", "WSRequestTrait", format!("{:?}", config));
        if uuid.trim().is_empty() {
            crate::log_e!(LogType::WSC; "reserve", "error", "ParameterEmpty");
        ...
        let mut entries = self.entries.write().map_err(|_| {
            crate::log_e!(LogType::WSC; "reserve", "error", "pending_lock_poisoned");
            NetError::InternalError
        })?;
        ...
        if entries.contains_key(uuid.as_str()) {
            crate::log_e!(LogType::WSC; "reserve", "uuid|error", uuid, "DuplicateRequestId");
            return Err(NetError::DuplicateRequestId);
        }
        if entries.len() >= self.max_entries {
            crate::log_e!(LogType::WSC; "reserve", "uuid|capacity|error", uuid, self.max_entries, "PendingRequestLimitReached");
        ...
        crate::log_s!(LogType::WSC; "reserve", "uuid|token|state|pending_count", uuid, token, "Queued", count);
        Ok((
            registration,
    ...
    pub(crate) fn mark_writing(
        ...
        crate::log_t!(LogType::WSC; "mark_writing", "uuid|token|attempt|generation", uuid, token, attempt, generation);
        let Ok(mut entries) = self.entries.write() else {
            crate::log_e!(LogType::WSC; "mark_writing", "error", "pending_lock_poisoned");
            return false;
        };
        let Some(entry) = entries.get_mut(uuid) else {
            crate::log_s!(LogType::WSC; "mark_writing", "uuid|state", uuid, "request_missing");
            return false;
        };
        if entry.token != token {
            crate::log_s!(LogType::WSC; "mark_writing", "uuid|token|state", uuid, token, "stale_token");
            return false;
        }
        ...
        entry.info.connection_generation = Some(generation);
        drop(entries);
        crate::log_s!(LogType::WSC; "mark_writing", "uuid|token|attempt|generation|state", uuid, token, attempt, generation, "Writing");
        true
    }

    pub(crate) fn mark_queued(&self, uuid: &str, token: u64) -> bool {
        crate::log_t!(LogType::WSC; "mark_queued", "uuid|token", uuid, token);
        let Ok(mut entries) = self.entries.write() else {
            crate::log_e!(LogType::WSC; "mark_queued", "error", "pending_lock_poisoned");
            return false;
        };
        let Some(entry) = entries.get_mut(uuid) else {
            crate::log_s!(LogType::WSC; "mark_queued", "uuid|state", uuid, "request_missing");
            return false;
        };
        if entry.token != token {
            crate::log_s!(LogType::WSC; "mark_queued", "uuid|token|state", uuid, token, "stale_token");
            return false;
        }
        entry.info.status = PendingRequestStatus::Queued;
        entry.info.connection_generation = None;
        drop(entries);
        crate::log_s!(LogType::WSC; "mark_queued", "uuid|token|state", uuid, token, "Queued");
        true
    }

    pub(crate) fn mark_sent(&self, uuid: &str, token: u64) -> Option<CancellationToken> {
        crate::log_t!(LogType::WSC; "mark_sent", "uuid|token", uuid, token);
        let Ok(mut entries) = self.entries.write() else {
            crate::log_e!(LogType::WSC; "mark_sent", "error", "pending_lock_poisoned");
            return None;
        };
        let entry = entries.get_mut(uuid)?;
        if entry.token != token {
            crate::log_s!(LogType::WSC; "mark_sent", "uuid|token|state", uuid, token, "stale_token");
            return None;
        }
        let written_at = tokio::time::Instant::now().into_std();
        if let Err(error) = entry.registration_state.mark_written(written_at) {
            crate::log_e!(LogType::WSC; "mark_sent", "error", format!("{error:?}"));
        ...
        drop(entries);
        crate::log_s!(LogType::WSC; "mark_sent", "uuid|token|state", uuid, token, "AwaitingResponse");
    ...
    pub(crate) fn remove_if_token(
        ...
        crate::log_t!(LogType::WSC; "remove_if_token", "uuid|token|error", uuid, token, format!("{:?}", error));
        let mut entries = self
            .entries
            .write()
            .map_err(|_| {
                crate::log_e!(LogType::WSC; "remove_if_token", "error", "pending_lock_poisoned");
            })
            .ok()?;
        ...
        crate::log_s!(LogType::WSC; "remove_if_token", "uuid|token|removed", uuid, token, request.is_some());
        request
    }
    ...
    pub(crate) fn response_dispatch_in_flight(&self, uuid: &str, token: u64) -> bool {
        crate::log_t!(LogType::WSC; "response_dispatch_in_flight", "uuid|token", uuid, token);
        let generation = self.entries.read().inspect_err(|_| { crate::log_e!(LogType::WSC; "response_dispatch_in_flight", "error", "lock_poisoned"); }).ok().and_then(|entries| {
            entries.get(uuid).and_then(|entry| {
                (entry.token == token)
        ...
        generation.is_some_and(|generation| {
            self.response_dispatches
                .lock().inspect_err(|_| { crate::log_e!(LogType::WSC; "response_dispatch_in_flight", "error", "lock_poisoned"); })
                .ok()
                .and_then(|dispatches| {
                    dispatches
                        .get(&generation)
    ...
    pub(crate) fn record_deferred_error(&self, uuid: &str, token: u64, error: NetError) -> bool {
        crate::log_t!(LogType::WSC; "record_deferred_error", "uuid|token|error", uuid, token, format!("{:?}", error));
        let Ok(mut entries) = self.entries.write() else {
            crate::log_e!(LogType::WSC; "record_deferred_error", "error", "pending_lock_poisoned");
            return false;
        };
        let Some(entry) = entries.get_mut(uuid) else {
            return false;
        ...
        if entry.token != token {
            crate::log_s!(LogType::WSC; "record_deferred_error", "uuid|token|state", uuid, token, "stale_token");
            return false;
        }
        let first_error = entry.record_deferred_error(error);
        drop(entries);
        crate::log_s!(LogType::WSC; "record_deferred_error", "uuid|token|first_error|state", uuid, token, format!("{:?}", first_error), "response_grace");
        true
    }
    ...
    pub(crate) fn fail_all_with_response_grace(
        ...
        crate::log_t!(LogType::WSC; "fail_all_with_response_grace", "error|response_dispatch_grace", format!("{:?}", error), format!("{:?}", response_dispatch_grace));
        if let Ok(mut entries) = self.entries.write().inspect_err(|_| {
            crate::log_e!(LogType::WSC; "fail_all_with_response_grace", "error", "lock_poisoned");
        }) {
        ...
        let (deferred, newly_scheduled) =
            if let Ok(mut dispatches) = self.response_dispatches.lock().inspect_err(|_| { crate::log_e!(LogType::WSC; "fail_all_with_response_grace", "error", "lock_poisoned"); }) {
        ...
        let drained = self
            .entries
            .write().inspect_err(|_| { crate::log_e!(LogType::WSC; "fail_all_with_response_grace", "error", "lock_poisoned"); })
            .map(|mut entries| {
            ...
            .unwrap_or_default();
        crate::log_s!(LogType::WSC; "fail_all_with_response_grace", "immediate_count|deferred_generations|new_timers", drained.len(), deferred.len(), newly_scheduled.len());
    ...
    pub(crate) fn fail_for_generation(
        ...
        crate::log_t!(LogType::WSC; "fail_for_generation", "generation|error|response_dispatch_grace", generation, format!("{:?}", error), format!("{:?}", response_dispatch_grace));
        if let Ok(mut entries) = self.entries.write().inspect_err(|_| {
            crate::log_e!(LogType::WSC; "fail_for_generation", "error", "lock_poisoned");
        }) {
        ...
        if defer {
            crate::log_s!(LogType::WSC; "fail_for_generation", "generation|state|timer_scheduled", generation, "response_grace", schedule);
            return;
        }
    ...
    pub(crate) fn begin_response_dispatch(&self, generation: u64) -> PendingResponseGuard {
        crate::log_t!(LogType::WSC; "begin_response_dispatch", "generation", generation);
        let mut dispatches = self
            .response_dispatches
            .lock()
            .unwrap_or_else(|poisoned| { crate::log_e!(LogType::WSC; "begin_response_dispatch", "error", "lock_poisoned_recovered"); poisoned.into_inner() });
        ...
        drop(dispatches);
        crate::log_s!(LogType::WSC; "begin_response_dispatch", "generation|in_flight", generation, in_flight);
    ...
    fn finish_response_dispatch(&self, generation: u64) {
        crate::log_t!(LogType::WSC; "finish_response_dispatch", "generation", generation);
        let terminal_error = if let Ok(mut dispatches) =
            self.response_dispatches.lock().inspect_err(|_| {
                crate::log_e!(LogType::WSC; "finish_response_dispatch", "error", "lock_poisoned");
            }) {
        ...
        if let Some(error) = terminal_error {
            crate::log_s!(LogType::WSC; "finish_response_dispatch", "generation|state", generation, "grace_completed");
            self.fail_generation_now(generation, error);
        }
        self.response_dispatch_changed.notify_waiters();
    }
    ...
    fn force_finish_response_dispatch(&self, generation: u64, fallback_error: NetError) {
        crate::log_t!(LogType::WSC; "force_finish_response_dispatch", "generation|fallback_error", generation, format!("{:?}", fallback_error));
        let error = self
            .response_dispatches
            .lock().inspect_err(|_| { crate::log_e!(LogType::WSC; "force_finish_response_dispatch", "error", "lock_poisoned"); })
            .ok()
            .and_then(|mut dispatches| dispatches.remove(&generation))
            .and_then(|dispatch| dispatch.terminal_error)
            .unwrap_or(fallback_error);
        crate::log_s!(LogType::WSC; "force_finish_response_dispatch", "generation|state|error", generation, "grace_expired", format!("{:?}", error));
        self.fail_generation_now(generation, error);
    }
    ...
    fn schedule_force_finish_response_dispatch(
        ...
        crate::log_t!(LogType::WSC; "schedule_force_finish_response_dispatch", "generation|error|grace", generation, format!("{:?}", error), format!("{:?}", grace));
        if grace.is_zero() {
            crate::log_s!(LogType::WSC; "schedule_force_finish_response_dispatch", "generation|state", generation, "grace_disabled");
        ...
        {
            crate::log_e!(LogType::WSC; "schedule_force_finish_response_dispatch", "generation|error", generation, spawn_error.to_string());
            self.force_finish_response_dispatch(generation, error);
        }
    }
    ...
    pub(crate) fn force_finish_all_response_dispatches(&self, fallback_error: NetError) {
        crate::log_t!(LogType::WSC; "force_finish_all_response_dispatches", "fallback_error", format!("{:?}", fallback_error));
        if let Ok(mut dispatches) = self.response_dispatches.lock().inspect_err(|_| { crate::log_e!(LogType::WSC; "force_finish_all_response_dispatches", "error", "lock_poisoned"); }) {
            dispatches.clear();
        }
        let failed = self
            .entries
            .write().inspect_err(|_| { crate::log_e!(LogType::WSC; "force_finish_all_response_dispatches", "error", "lock_poisoned"); })
            .map(|mut entries| entries.drain().map(|(_, entry)| entry).collect::<Vec<_>>())
            .unwrap_or_default();
        crate::log_s!(LogType::WSC; "force_finish_all_response_dispatches", "failed_count", failed.len());
        for entry in failed {
    ...
    pub(crate) async fn wait_for_deferred_responses_until(
        ...
        crate::log_t!(LogType::WSC; "wait_for_deferred_responses_until", "deadline", format!("{:?}", deadline));
        ...
        loop {
            ...
            if !self.has_pending_entries() {
                crate::log_s!(LogType::WSC; "wait_for_deferred_responses_until", "state", "all_responses_finished");
                return;
            }
            if tokio::time::Instant::now() >= deadline {
                crate::log_s!(LogType::WSC; "wait_for_deferred_responses_until", "state", "grace_deadline_reached");
                return;
            }
            tokio::select! {
                biased;
    ...
    fn has_pending_entries(&self) -> bool {
        crate::log_t!(LogType::WSC; "has_pending_entries");
        self.entries
            .read()
            .map(|entries| !entries.is_empty())
            .unwrap_or_else(|_| {
                crate::log_e!(LogType::WSC; "has_pending_entries", "error", "pending_lock_poisoned");
                true
            })
    }

    fn fail_generation_now(&self, generation: u64, error: NetError) {
        crate::log_t!(LogType::WSC; "fail_generation_now", "generation|error", generation, format!("{:?}", error));
        let failed = if let Ok(mut entries) = self.entries.write().inspect_err(|_| {
            crate::log_e!(LogType::WSC; "fail_generation_now", "error", "lock_poisoned");
        }) {
            let failed = entries
                .iter()
                .filter_map(|(uuid, entry)| {
        ...
        crate::log_s!(LogType::WSC; "fail_generation_now", "generation|failed_count", generation, failed.len());
        for entry in failed {
            entry.complete(Err(error));
        }
        self.response_dispatch_changed.notify_waiters();

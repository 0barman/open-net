...
impl PendingRequestEntry {
    ...
    pub(in crate::api::wsc) fn complete(
        ...
        match result {
            Ok(()) => {
                crate::log_s!(LogType::WSC; "complete", "uuid|token|state", self.info.uuid, self.token, "response_claimed")
            }
            Err(crate::NetError::Cancelled) => {
                crate::log_s!(LogType::WSC; "complete", "uuid|token|state", self.info.uuid, self.token, "cancelled")
            }
            Err(error) => {
                crate::log_e!(LogType::WSC; "complete", "uuid|token|error", self.info.uuid, self.token, format!("{:?}", error))
            }
        }
        ...
        if let Some(completion_tx) = self.completion_tx.take() {
            if completion_tx.send(result).is_err() {
                crate::log_s!(LogType::WSC; "complete", "uuid|token|state", self.info.uuid, self.token, "completion_receiver_dropped");
            }
        }
        self.request
    }

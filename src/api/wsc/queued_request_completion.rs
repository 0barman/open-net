...
impl QueuedRequestCompletion {
    pub(crate) fn new(
        ...
        crate::log_t!(LogType::WSC; "new", "write_result|pending_completion", "oneshot::Receiver", "PendingRequestCompletion");
        Self {
    ...
    pub async fn wait_until_written(self) -> Result<PendingRequestCompletion, NetError> {
        crate::log_t!(LogType::WSC; "wait_until_written");
        match self.write_result.await {
            Ok(Ok(())) => {
                crate::log_s!(LogType::WSC; "wait_until_written", "request_id|state", self.request_id, "write_or_response_confirmed");
                Ok(self.pending_completion)
            }
            Ok(Err(NetError::Cancelled)) => {
                crate::log_s!(LogType::WSC; "wait_until_written", "request_id|state", self.request_id, "cancelled");
                Err(NetError::Cancelled)
            }
            Ok(Err(error)) => {
                crate::log_e!(LogType::WSC; "wait_until_written", "request_id|error", self.request_id, format!("{:?}", error));
                Err(error)
            }
            Err(_) => {
                crate::log_e!(LogType::WSC; "wait_until_written", "request_id|error", self.request_id, "EngineDropped");

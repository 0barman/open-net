...
impl WebSocketClient {
    ...
    pub(crate) fn from_inner(inner: Arc<WSClientInner>) -> Self {
        crate::log_t!(LogType::WSC; "from_inner");
        Self { inner }
    }
}
...
impl WebSocketClient {
    ...
    pub fn register_web_socket_client_data_receive_listener(
        ...
        crate::log_t!(LogType::WSC; "register_web_socket_client_data_receive_listener", "listener", "callback");
        self.inner.register_data_listener(listener);
    }

    pub fn unregister_web_socket_client_data_receive_listener(&self) {
        crate::log_t!(LogType::WSC; "unregister_web_socket_client_data_receive_listener");
    ...
    pub fn register_web_socket_client_connect_status_listener(
        ...
        crate::log_t!(LogType::WSC; "register_web_socket_client_connect_status_listener", "listener", "callback");
        self.inner.register_status_listener(listener);
    }

    pub fn unregister_web_socket_client_connect_status_listener(&self) {
        crate::log_t!(LogType::WSC; "unregister_web_socket_client_connect_status_listener");
    ...
    pub async fn connect(&self, url: &str) -> Result<(), NetError> {
        crate::log_t!(LogType::WSC; "connect", "url", crate::common::log::summary::url(url));
    ...
    pub async fn connect_with_options(
        ...
        crate::log_t!(LogType::WSC; "connect_with_options", "url|header_count|has_header_provider|reconnect", crate::common::log::summary::url(url), options.headers.len(), options.header_provider.is_some(), format!("{:?}", options.reconnect));
        let result: Result<(), NetError> = async {
        ...
        .await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "connect_with_options", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "send", "request_type", std::any::type_name::<R>());
        let result: Result<(), NetError> = async {
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "send_with_config", "config|request_type", format!("{:?}", config), std::any::type_name::<R>());
        let result: Result<(), NetError> =
            async { self.inner.send(Arc::new(request), config).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "send_with_completion", "config|request_type", format!("{:?}", config), std::any::type_name::<R>());
    ...
    pub async fn send_shared_with_completion(
        ...
        crate::log_t!(LogType::WSC; "send_shared_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<PendingRequestCompletion, NetError> =
            async { self.inner.send_with_completion(request, config).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_shared_with_completion", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_boxed_with_completion(
        ...
        crate::log_t!(LogType::WSC; "send_boxed_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<PendingRequestCompletion, NetError> = async {
            self.inner
        ...
        .await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_boxed_with_completion", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_shared(
        ...
        crate::log_t!(LogType::WSC; "send_shared", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<(), NetError> = async { self.inner.send(request, config).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_shared", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_boxed(
        ...
        crate::log_t!(LogType::WSC; "send_boxed", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<(), NetError> =
            async { self.inner.send(Arc::from(request), config).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_boxed", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "try_send", "request_type", std::any::type_name::<R>());
        let result: Result<(), NetError> =
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "try_send_with_config", "config|request_type", format!("{:?}", config), std::any::type_name::<R>());
        let result: Result<(), NetError> = self.inner.try_send(Arc::new(request), config);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn try_send_shared(
        ...
        crate::log_t!(LogType::WSC; "try_send_shared", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<(), NetError> = self.inner.try_send(request, config);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_shared", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn try_send_boxed(
        ...
        crate::log_t!(LogType::WSC; "try_send_boxed", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<(), NetError> = self.inner.try_send(Arc::from(request), config);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_boxed", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    where
        R: WSRequestTrait,
    {
        crate::log_t!(LogType::WSC; "try_send_with_completion", "config|request_type", format!("{:?}", config), std::any::type_name::<R>());
    ...
    pub fn try_send_shared_with_completion(
        ...
        crate::log_t!(LogType::WSC; "try_send_shared_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<QueuedRequestCompletion, NetError> =
            self.inner.try_send_with_completion(request, config);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_shared_with_completion", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn try_send_boxed_with_completion(
        ...
        crate::log_t!(LogType::WSC; "try_send_boxed_with_completion", "config|request_type", format!("{:?}", config), "dyn WSRequestTrait");
        let result: Result<QueuedRequestCompletion, NetError> = {
            self.inner
                .try_send_with_completion(Arc::from(request), config)
        };
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_boxed_with_completion", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_message(&self, body: WsBody) -> Result<(), NetError> {
        crate::log_t!(LogType::WSC; "send_message", "body_bytes", match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> = async {
        ...
        .await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_message", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_message_with_config(
        ...
        crate::log_t!(LogType::WSC; "send_message_with_config", "config|body_bytes", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> =
            async { self.inner.send_untracked(body, config, false).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_message_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }

    /// 同步、非阻塞地尝试入队一条普通单向消息。
    pub fn try_send_message(&self, body: WsBody) -> Result<(), NetError> {
        crate::log_t!(LogType::WSC; "try_send_message", "body_bytes", match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> =
            self.try_send_message_with_config(body, WSRequestConfig::default());
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_message", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn try_send_message_with_config(
        ...
        crate::log_t!(LogType::WSC; "try_send_message_with_config", "config|body_bytes", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> = self.inner.try_send_untracked(body, config, false);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_message_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_urgent_message(&self, body: WsBody) -> Result<(), NetError> {
        crate::log_t!(LogType::WSC; "send_urgent_message", "body_bytes", match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> = async {
        ...
        .await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_urgent_message", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub async fn send_urgent_message_with_config(
        ...
        crate::log_t!(LogType::WSC; "send_urgent_message_with_config", "config|body_bytes", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> =
            async { self.inner.send_untracked(body, config, true).await }.await;
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "send_urgent_message_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }

    /// 同步、非阻塞地尝试入队一条紧急单向消息。
    pub fn try_send_urgent_message(&self, body: WsBody) -> Result<(), NetError> {
        crate::log_t!(LogType::WSC; "try_send_urgent_message", "body_bytes", match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> =
            self.try_send_urgent_message_with_config(body, WSRequestConfig::default());
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_urgent_message", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
    ...
    pub fn try_send_urgent_message_with_config(
        ...
        crate::log_t!(LogType::WSC; "try_send_urgent_message_with_config", "config|body_bytes", format!("{:?}", config), match &body { WsBody::Text(v) => v.len(), WsBody::Binary(v) => v.len() });
        let result: Result<(), NetError> = self.inner.try_send_untracked(body, config, true);
        result.inspect_err(|error| {
            crate::log_e!(LogType::WSC; "try_send_urgent_message_with_config", "error|error_code", format!("{error:?}"), *error as i32);
        })
    }
